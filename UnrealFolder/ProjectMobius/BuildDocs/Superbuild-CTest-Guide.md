...
## Why We Did This
We wanted a quick, repeatable sanity check after the superbuild finishes: make sure Assimp's unit suite builds and runs, with readable output for both engineers (XML) and non-devs (plain text).
...
## How It Runs
- CTest target: `superbuild_tests` depends on staging (`assimp_stage`). On build, it runs `ctest` (with `-C ${SUPERBUILD_CTEST_CONFIG}` when needed) and logs to `_superbuild/Testing/superbuild-ctest.log`.
- Assimp checks:
...
- `_superbuild/Testing/superbuild-ctest.log` - Assimp unit test run.
