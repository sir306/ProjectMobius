...
The Unreal project lives in `UnrealFolder/ProjectMobius/` with `Source/`, `Content/`, `Config/`, `Plugins/`, `UnitTestSampleData/`, and `ProjectMobius.uproject`. Root-level asset and data directories include `ImportedOpenSourceAssets/`, `TestData/`, and `DemoProgressVideos/`. Follow the UE-specific rules in `UnrealFolder/ProjectMobius/AGENTS.md` when changing code inside the Unreal project.
...
Treat third-party folders (`ImportedOpenSourceAssets/`, plugin `ThirdParty/` directories) as vendored; avoid edits unless you are updating licenses or replacing upstream drops. If you add assets or tooling, update `THIRD-PARTY-LICENSES.md`. The legacy WebSocket server path is deprecated; prefer the IPC subsystem in `UnrealFolder/ProjectMobius/Source/MobiusCore` for tool integration.
